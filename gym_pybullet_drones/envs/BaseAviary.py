...
class BaseAviary(gym.Env, MathematicalFlock):
    ...
    def getCattleIds(self):
        ...
        -------
        ndarray:
            (NUM_CATTEL,)-shaped array of ints containing the cattel' ids.

        """
        return self.CATTEL_IDS
    ...
    def _housekeeping(self):
        ...
        #### Initialize the cows kinemaatic information ##########
        self.cattle_pos = np.zeros((self.NUM_CATTLE, 3))
        self.cattle_quat = np.zeros((self.NUM_CATTLE, 4))
        ...
        self.CattleCentroidMarker = p.createMultiBody(
            baseMass=0,  # static
            baseVisualShapeIndex=self.CattleCentroidMarkerVisual,
            basePosition=[0, 0, -999],  # initially hidden
            physicsClientId=self.CLIENT
        )
        
        #Drone centorid marker
        self.DroneCentroidMarkerVisual = p.createVisualShape(
            shapeType=p.GEOM_SPHERE,
            radius=0.1,
            rgbaColor=[0,1,0,1],  # red, fully opaque
            physicsClientId=self.CLIENT
        )
        self.DroneCentroidMarker = p.createMultiBody(
            baseMass=0,               # mass 0 = static
            baseVisualShapeIndex=self.DroneCentroidMarkerVisual,
            basePosition=[0,0,-999],
            physicsClientId=self.CLIENT
        )
        
        #Goal Marker
        self.HerdGoalMarkerVisual = p.createVisualShape(
            shapeType=p.GEOM_SPHERE,
            radius=0.2,
            rgbaColor=[0,1,0,1],  # green sphere
            physicsClientId=self.CLIENT
        )

        self.HerdGoalMarker = p.createMultiBody(
            baseMass=0,  # static
            baseVisualShapeIndex=self.HerdGoalMarkerVisual,
            basePosition=[7, -5, 1.2],
            physicsClientId=self.CLIENT
        )
    ...
    def _getCowStateVector(self, nth_cow):
        """Returns the state vector of the n-th cow.

        Parameters
        ----------
        nth_cow : int
            The ordinal number/position of the desired cow.

        Returns
        -------
        ndarray
            (16,)-shaped array of floats containing the state vector of the n-th cow:
            [pos(3), quat(4), rpy(3), vel(3), ang_v(3)]
        """
        state = np.hstack([self.cattle_pos[nth_cow, :],
                        self.cattle_quat[nth_cow, :],
                        self.cattle_rpy[nth_cow, :],
                        self.cattle_vel[nth_cow, :],
                        self.cattle_ang_v[nth_cow, :]])
